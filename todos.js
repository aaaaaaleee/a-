...
router.delete('/:id', totalMiddleware, async (ctx, next) => {
  const result = await deleteTodo({_id:ctx.params.id }   /*
...
router.patch('/:id', koaBody(), totalMiddleware,logger, async (ctx, next) => {

  const result = await updateTodo(  ctx.query , { title: ctx.request.body.title })
  if (!result) {
    throw new NotFoundError(`todo with ID ${ctx.params.id} is not found`)
  }
  ctx.set('Location', `/api/v1/todos/${id}`)
